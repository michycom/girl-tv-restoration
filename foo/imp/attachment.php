<?php
/**
 * $Horde: imp/attachment.php,v 2.5.10.19 2007/01/02 13:54:53 jan Exp $
 *
...
if ($conf['compose']['link_attachments_notify']) {
    if ($vfsroot->exists($full_path, $file_name . '.notify')) {
        ...
        if (is_a($res, 'PEAR_Error')) {
             ...
             * locale for the notification message. */
            include_once 'Horde/Prefs.php';
            $mail_prefs = &Prefs::singleton($conf['prefs']['driver'],
                                            'horde', $mail_user);
            $mail_prefs->retrieve();
            ...
            if (strlen($mail_address) > 2) {
                ...
                NLS::setLang($mail_prefs->getValue('language'));
