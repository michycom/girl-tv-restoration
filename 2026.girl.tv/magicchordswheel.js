...
(function(){
  ...
  // Quintenzirkel (äußerer Ring) mit Paralleltonarten
  const CIRCLE_KEYS = [
    { maj:"C",  min:"Am" },
    { maj:"G",  min:"Em" },
    { maj:"D",  min:"Bm" },
  ...
  // einfache Abhängigkeitsregeln im Stern: fi braucht mi, ti braucht te etc.
  // (nur „passende“ Kombinationen) – lässt sich später weiter ausbauen
  const STAR_DEP_RULES = {
    1:[0],  // di → do
    6:[4],  // fi → mi
    11:[10] // ti → te
  };
  ...
  // Solo-Ansicht für Root: andere Roots ausgeblendet
  let isRootSolo = false;

  // Manuelle Markierungen im Stern (do/di/re… klickbar)
  const starManual = {}; // starManual[rootIndex][degree] = bool
  ...
  // Geometrie – etwas mehr Abstand zwischen den Root-Kreisen
  const centerX = 260;
  const centerY = 260;
  const rootRadius = 200;       // vorher 180/195 – jetzt mehr Raum
  const rootCircleRadius = 45;
  const keyRadius = 230;        // äußerer Ring (Quintenzirkel)
  ...
  // Mouseleave → Solo-Modus wieder aus
  ...
  function buildWheel(){
    ...
    for(let i=0;i<ROOTS.length;i++){
      ...
      rootCircle.addEventListener("click", ()=>{
        // Klick = Root setzen und Solo-Ansicht toggeln
        if(currentRootIndex === i && isRootSolo){
          isRootSolo = false;
        }else{
          currentRootIndex = i;
          isRootSolo = true;
      ...
      // Root-star (chromatische Solmisation – do/di/re… in Kreisen)
      const starGroup = makeEl("g",{
        class:"root-star-group",
        "data-root-index": String(i)
      });
      const rayLen = rootCircleRadius * 1.35; // Strahlenlänge

      if(!starNodes[i]) starNodes[i] = {};
      if(!starManual[i]) starManual[i] = {};
      ...
      for(let d=0; d<12; d++){
        ...
        // Klick = manuelle Markierung des Solmisation-Stücks mit Abhängigkeiten
        node.addEventListener("click",(ev)=>{
          ev.stopPropagation();
          toggleStarManual(i, d);
          updateVisuals();
        });
  ...
  // --- Stern-Logik: nur „zulässige“ Kombinationen ---
  function toggleStarManual(rootIdx, degree){
    if(!starManual[rootIdx]) starManual[rootIdx] = {};
    const current = !!starManual[rootIdx][degree];
    if(current){
      // ausschalten: und alle, die diese Note als Voraussetzung haben, auch aus
      starManual[rootIdx][degree] = false;
      for(const dStr in STAR_DEP_RULES){
        const d = Number(dStr);
        const deps = STAR_DEP_RULES[d] || [];
        if(deps.includes(degree) && starManual[rootIdx][d]){
          starManual[rootIdx][d] = false;
        }
      }
    }else{
      // einschalten + abhängige Grundtöne erzwingen
      starManual[rootIdx][degree] = true;
      const deps = STAR_DEP_RULES[degree] || [];
      deps.forEach(dep=>{
        starManual[rootIdx][dep] = true;
      });
    }
  }

  // --- Harmonielehre-Sanitisierung der Akkord-Dots ---
  function sanitizeHarmony(){
    // 1. Powerchord dominiert: nur 1 + 5
    if(power5){
      triadQuality = null;
      for(const k in ext){
        ext[k] = false;
    ...
    // 2. 7 und maj7 exklusiv (bereits im Click, hier nur nochmal sicher)
    if(ext["7"] && ext["maj7"]){
      // Priorität: maj7 vor 7
      ext["7"] = false;
    }

    // 3. 13 ohne Septime ist theoretisch „unsauber“ → 7 automatisch hinzufügen
    if(ext["13"] && !ext["7"] && !ext["maj7"]){
      ext["7"] = true;
    }

    // 4. Dim- und Aug-Dreiklang: keine 6/13 (vereinfachte Regel)
    if(triadQuality === "dim" || triadQuality === "aug"){
      ext["6"] = false;
      ext["13"] = false;
    }

    // 5. Sus-Akkorde eher ohne maj7 (vereinfachte Regel)
    if(triadQuality === "sus2" || triadQuality === "sus4"){
      if(ext["maj7"]){
        ext["maj7"] = false;
      }
    }
  ...
  function setKey(keyIdx){
    ...
    isRootSolo = false; // bei Key-Wechsel alle Roots sichtbar
    sanitizeHarmony();
    updateAll();
  }
  ...
  function updateVisuals(){
    // Root-Gruppen (Aktiv + Fade)
    rootGroups.forEach((g, idx)=>{
      if(!g) return;
      const isActive = idx === currentRootIndex;
      g.classList.toggle("active-root", isActive);
      g.classList.toggle("faded-root", isRootSolo && idx !== currentRootIndex);

      // aktive Gruppe visuell nach vorne holen (kein echtes z-index in SVG → wir setzen ihr eine stärkere Aura)
      if(isActive){
        g.classList.add("root-emphasis");
      }else{
        g.classList.remove("root-emphasis");
      }
    });
    ...
    for(const rootIdx in starNodes){
      ...
      for(let d=0; d<12; d++){
        ...
        if(!node) continue;
        const manual = starManual[idxNum] && !!starManual[idxNum][d];
        const on = isActiveRoot && (lastNormIntervals.includes(d) || manual);
