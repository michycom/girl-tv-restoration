    ...
    build(octaveRange) {
      ...
      for (let octave = octaveRange[0]; octave <= octaveRange[1]; octave++) {
        ...
        this.notes.forEach((note, index) => {
          ...
          label.innerText = note.includes('#') ? key.id.replace('#', '♯') :  key.id;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  startButton.addEventListener('click', function() {
      if (!window.customAudioContext) {
          // Initialisiere den AudioContext bei der ersten Benutzerinteraktion
          window.customAudioContext = new (window.AudioContext || window.webkitAudioContext)();

          // Initialisierung der Klaviertastatur und anderer Komponenten
          initializeApplication();
      } else {
          // Wenn der AudioContext bereits existiert, stelle sicher, dass er nicht im "suspended" Zustand ist
          window.customAudioContext.resume();
      }

      // Verstecke den Start-Button, nachdem die Anwendung gestartet wurde
      startButton.style.display = 'none';
  });
  
  // Füge den Event-Listener für die Klaviertasten außerhalb des Start-Button-Event-Listeners hinzu
  document.addEventListener('click', function(event) {
      if (!event.target.closest('.key')) {
          // Wiedergabe des zuletzt gespielten Tons, falls außerhalb der Tasten geklickt wurde
...
function initializeApplication() {
  ...
  // Initialisiere den Trainer mit der neu instanziierten Klasse EarTrainer
