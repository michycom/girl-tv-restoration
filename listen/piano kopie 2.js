...
class NoteGenerator {
    ...
    synthesizer(note_hight_octave) {
      ...
      var real = new Float32Array([0, 1.5, 0.5, 0.2]);
      ...
      gainNode.gain.exponentialRampToValueAtTime(0.001, this.audioContext.currentTime + 1); // Decay zu fast 0 in einer Sekunde
  
      oscillator.connect(gainNode);
      gainNode.connect(this.audioContext.destination);
      oscillator.start();
      oscillator.stop(this.audioContext.currentTime + 2); // Der Ton spielt für 1 Sekunde
    }
 ...
 class PianoBuilder {
    constructor(containerId) {
     ...
     build(octaveRange) {
        ...
        let [startOctave, endOctave] = octaveRange;
        ...
        for (let octave = startOctave; octave <= endOctave; octave++) {
            this.notes.forEach((note) => {
              ...
              key.id = `${note}${octave}`;
              key.innerText = `${note}${octave}`; // Tonname hinzufügen
              key.style.position = 'relative'; // Ermöglicht die Positionierung des Texts
              const label = document.createElement('span');
              label.style.position = 'absolute';
              label.style.right = '5px';
              label.style.bottom = '5px';
              label.style.fontSize = '8px';
              label.style.color = 'rgba(255, 255, 255, 0.7)'; // Heller Text für dunkle Tasten, oder du passt die Farbe basierend auf der Taste an
