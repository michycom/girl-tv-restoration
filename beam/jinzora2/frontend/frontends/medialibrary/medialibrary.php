...
?>
<div id="left_box">
<select name="<?php echo jz_encode("jz_path"); ?>" size="14" class="full jz_select" style="height:210px;" onChange="submitForm(this.form,'<?php echo $this_page; ?>')">
<?php
	if ($backstring !== false) {
		echo "<option value=\"".jz_encode($backstring)."\">".word('[ Back ]')."</option";
	}
	foreach ($left as $el) {
		echo "<option value=\"".jz_encode($el->getPath("String"))."\"";
		if ($el->getPath() == $leftpath) {
			echo " selected";
		}
		echo ">".$display->returnShortName($el->getName(),35)."</option>";
	}
?>
</select>
</div>
...
<div id="track_box">
<input type="hidden" name="<?php echo jz_encode("jz_path"); ?>" value="<?php echo jz_encode($node->getPath("String")); ?>">
<select name="jz_list[]" size="18" class="full jz_select" ondblclick="if (submitPlaybackForm(this,'<?php echo $this_page; ?>')) submit()" multiple>
<?php
	// this should be fixed to however playlists get loaded.
	foreach ($songs as $el) {
		echo "<option value=\"".jz_encode($el->getPath("String"))."\">".$display->returnShortName($el->getName(),75)."</option>";
	}
?>
</select>
</div>
...
<input type="submit" name="<?php echo jz_encode("sendList"); ?>" value="<?php echo word('Play'); ?>" onClick="return submitPlaybackForm(this,'<?php echo $this_page; ?>')" class="jz_submit">
<input type="submit" name="<?php echo jz_encode("sendPath"); ?>" value="<?php echo word('Play All'); ?>" onClick="return submitPlaybackForm(this,'<?php echo $this_page; ?>')" class="jz_submit">
<input type="submit" name="<?php echo jz_encode("sendPathRandom"); ?>" value="<?php echo word('Random Play All'); ?>" onClick="return submitPlaybackForm(this,'<?php echo $this_page; ?>')" class="jz_submit">
<!--<input type="submit" name="<?php echo jz_encode("info"); ?>" value="Get Info" class="jz_submit">--!>
<?php } ?>
</div></td>
<td nowrap><div align="right">
<input type="submit" name="<?php echo jz_encode("addList"); ?>" value="<?php echo word('Add'); ?>" class="jz_submit" onClick="return submitPlaybackForm(this,'<?php echo $this_page; ?>')">
<input type="submit" name="<?php echo jz_encode("addPath"); ?>" value="<?php echo word('Add All'); ?>" class="jz_submit" onClick="return submitPlaybackForm(this,'<?php echo $this_page; ?>')">
</div></td></tr>
</form>
</table>
<td>
...
function keepVars($GET) {
  if (isset($GET['query']))
    echo "<input type=\"hidden\" name=\"query\" value=\"".$GET['query']."\">";
  if ($GET['how'] == "filter")
    echo "<input type=\"hidden\" name=\"" . jz_encode("how") . "\" value=\"" . jz_encode("filter") . "\">";
  if ($GET['how'] == "search")
    echo "<input type=\"hidden\" name=\"" . jz_encode("how") . "\" value=\"" . jz_encode("search") . "\">";
