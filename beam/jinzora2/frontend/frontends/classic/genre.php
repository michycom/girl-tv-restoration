<?php if (!defined(JZ_SECURE_ACCESS)) die ('Security breach detected.');
	...
	* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	...
	function drawPage(&$node){
		...
		$enable_discussion, $img_discuss, $show_sub_numbers, $disable_random, $info_level, $days_for_new,
		...
		for ($i=0; $i < count($retArray); $i++){
			...
			if ($show_all_checkboxes == "true" and $enable_playlist <> "false"){
				jzTDOpen("1","left","top","","0");
				echo '<input class="jz_checkbox" name="track-'. $i. '" type=checkbox value="'. $root_dir. $media_dir. '/'. rawurlencode($genre). '/'. urlencode($retArray[$i]->getPath("String")). '/">'; 
				jzTDClose();
