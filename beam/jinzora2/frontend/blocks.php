			...
			<tr>
							...
							<select multiple name="jbjumpto[]" id="jukeboxJumpToSelect" class="jz_select" size="6" style="width:275px;"<?php if ($func['jump']){ echo 'ondblclick="setJbFormCommand(\'jumpto\'); sendJukeboxForm(); return false;"'; }?>>
								<?php
									for ($i=0; $i < count($fullList); $i++){
										echo '<option value="'. $i. '"';
										if ((false !== array_search($i,$selected)) || (count($selected) == 0 && $curTrackNum == $i)) {
											echo " selected ";
										}
										if ($curTrackNum == $i) { 
				...
				<td width="40%" valign="top">
					...
					<?php
						...
						$myTrack = new jzMediaTrack($filePath);
						$meta = $myTrack->getMeta();
						$lyrics = $meta['lyrics'];
						$keys= array_keys($meta);
						if ($lyrics == null) {
							echo "No lyrics for this song, add them <a href=\"./popup.php?action=popup&ptype=trackinfo&jz_path=" . urlencode($filePath) . "\" target=\"_blank\" onclick=\"openPopup(this, 375, 650); return false;\">here</a>";
						} else {
							echo "<textarea rows=9 class=\"jz_input\" style=\"width:100%;\" readonly>" . (utf8_decode(utf8_decode($meta['lyrics']))) . "</textarea>";
						}
					?>
				</td>
