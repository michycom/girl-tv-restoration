<?php if (!defined(JZ_SECURE_ACCESS)) die ('Security breach detected.');
	...
	*/
	function playlist($playlist){
		...
		exec ("/usr/bin/killall -USR1 sc_trans_linux > /dev/null 2>&1 &");
		if ($add == "replace" || $add == "begin") {
		  exec ("/usr/bin/killall -WINCH sc_trans_linux  > /dev/null 2>&1 &");  
		}
	...
	function control($command, $goBack = true){
		...
		switch ($command){
			...
			case "play":
				// Reload playlist
				exec ("/usr/bin/killall -USR1 sc_trans_linux > /dev/null 2>&1 &");		
				writeLogData("messages","Shoutcast reload with: killall -USR1 sc_trans_linux");
				return;
			break;
			case "stop":
                                exec ("/usr/bin/killall -TERM sc_trans_linux > /dev/null 2>&1 &");
                                writeLogData("messages","Shoutcast stop with: killall -TERM sc_trans_linux");
				return;
			break;
			case "pause":
				return;
			...
			case "next":
				exec ("/usr/bin/killall -WINCH sc_trans_linux > /dev/null 2>&1 &");		
				writeLogData("messages","Shoutcast next with: killall -WINCH sc_trans_linux");
				return;
			break;
			case "volume":
			...
			case "clear":
 				// Reload actualy	
				exec ("/usr/bin/killall -USR1 sc_trans_linux > /dev/null 2>&1 &");		
				writeLogData("messages","Shoutcast clear with: killall -USR1 sc_trans_linux");
				return;
			break;
			case "random_play":
				exec ("/usr/bin/killall -USR2 sc_trans_linux > /dev/null 2>&1 &");		
				writeLogData("messages","Shoutcast random with: killall -USR2 sc_trans_linux");
				return;
			break;
			case "addwhere":
